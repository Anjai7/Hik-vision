...
| **Fingerprint** | `GET` | `/ISAPI/AccessControl/FingerPrint/capabilities` | XML / JSON | Fingerprint scanner capacity and biometric encryption flags. | None |
| **Fingerprint** | `GET` | `/ISAPI/AccessControl/FingerPrint/Count?format=json` | JSON | Query count of enrolled fingerprints on device. | Query: `?format=json` |
| **Fingerprint** | `POST` | `/ISAPI/AccessControl/CaptureFingerPrint` | XML / JSON | Interactive fingerprint enrollment via device optical reader. | Interactive capture command |
